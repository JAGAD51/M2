  ...
  private String name;
	...
	private String calc(String input) {
		...
		while(regular.length() != 0) {
			int randomIndex = (int)(Math.random() * regular.length());
			randomized += regular.charAt(randomIndex);
			regular = regular.substring(0, randomIndex) + regular.substring(randomIndex + 1);
		}
	  return randomized;
	}
